...
async fn test_request_id_preserved_when_provided() {
    ...
    assert_eq!(returned_request_id, provided_request_id, "Request ID should match the provided one");
}
...
async fn test_request_id_different_for_each_request() {
    ...
    let request_id1 = res1.headers().get("x-request-id").unwrap().to_str().unwrap();

    // Make second request
    let req2 = Request::get("/health_check").body(Body::empty()).unwrap();
    let res2 = app.request(req2).await;
    let request_id2 = res2.headers().get("x-request-id").unwrap().to_str().unwrap();

    // Verify they are different
    assert_ne!(request_id1, request_id2, "Each request should have a unique request ID");
}
...
async fn test_request_id_case_insensitive_header() {
    ...
    assert_eq!(returned_request_id, provided_request_id, "Request ID should be preserved regardless of header case");
}
